...
def assess_hazard(my_health, hazard_damage, hazard_num, deadly_moves, risky_moves):

    layered_damage = hazard_damage * hazard_num

    if layered_damage >= my_health:
        move_tier = deadly_moves
    else:
        move_tier = risky_moves
    
    print(f"Found a hazard, count: {hazard_num} | My health: {my_health} | Hazard damage: {hazard_damage} | Layered damage: {layered_damage}")
...
def avoid_hazards(my_head, my_health, hazards, hazard_damage, possible_moves, risky_moves):
    ...
    #if (count := hazards.count(...)) >= 1:

    if (hazard_num := hazards.count(find_square({"x": my_head["x"]-1, "y": my_head["y"]}))) >= 1:
        move_tier = assess_hazard(my_health, hazard_damage, hazard_num, deadly_moves, risky_moves)
        delete_move("left", possible_moves)
    ...
    if (hazard_num := hazards.count(find_square({"x": my_head["x"], "y": my_head["y"]-1}))) >= 1:
        move_tier = assess_hazard(my_health, hazard_damage, hazard_num, deadly_moves, risky_moves)
        delete_move("down", possible_moves)
